...
pub trait GoalEngineTrait {
    ...
    async fn suggest_goals(
        ...
        activities: &[Activity],
    ) -> IntelligenceResult<Vec<GoalSuggestion>>;
    ...
    async fn track_progress(
        ...
        goal: &Goal,
        activities: &[Activity],
    ) -> IntelligenceResult<ProgressReport>;
    ...
    ) -> IntelligenceResult<Option<GoalAdjustment>>;

    /// Create milestone structure for a goal
    async fn create_milestones(&self, goal: &Goal) -> IntelligenceResult<Vec<Milestone>>;
}
...
impl<S: IntelligenceStrategy> GoalEngineTrait for AdvancedGoalEngine<S> {
    ...
    async fn track_progress(
        ...
        let milestones = self
            ...
            .await
            .map_err(|e| IntelligenceError::internal(format!("Milestone creation failed: {e}")))?;
