...
pub fn calculate_tdee(
    ...
    if bmr <= 0.0 {
        return Err(IntelligenceError::invalid_input("BMR must be positive"));
    }
...
pub fn calculate_protein_needs(
    ...
    if weight_kg <= 0.0 {
        return Err(IntelligenceError::invalid_input("Weight must be positive"));
    }
...
pub fn calculate_fat_needs(
    ...
    if tdee <= 0.0 {
        return Err(IntelligenceError::invalid_input("TDEE must be positive"));
    }
...
) -> Result<DailyNutritionNeeds, IntelligenceError> {
...
) -> Result<NutrientTimingPlan, IntelligenceError> {
    if weight_kg <= 0.0 {
        return Err(IntelligenceError::invalid_input("Weight must be positive"));
    }
