...
impl SleepAnalyzer {
    ...
    ///
    /// # Errors
    /// Returns `IntelligenceError` if sleep data is invalid (negative values, impossible percentages)
    pub fn calculate_sleep_quality(
        sleep: &SleepData,
        config: &SleepRecoveryConfig,
    ) -> Result<SleepQualityScore, IntelligenceError> {
        // Validate input
    ...
    pub fn analyze_hrv_trends(
        ...
        config: &SleepRecoveryConfig,
    ) -> Result<HrvTrendAnalysis, IntelligenceError> {
