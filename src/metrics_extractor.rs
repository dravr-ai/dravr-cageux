...
impl SafeMetricExtractor {
    ...
    ) -> IntelligenceResult<Vec<(DateTime<Utc>, f64)>> {
    ...
    pub fn calculate_metric_summary(
        activities: &[Activity],
        metric_type: MetricType,
    ) -> IntelligenceResult<MetricSummary> {
        let values = Self::extract_metric_values(activities, metric_type)
            .map_err(|e| IntelligenceError::internal(format!("Metric extraction failed: {e}")))?;
        let metric_values: Vec<f64> = values.into_iter().map(|(_, value)| value).collect();

        if metric_values.is_empty() {
            return Err(IntelligenceError::internal("No values to summarize"));
        }
