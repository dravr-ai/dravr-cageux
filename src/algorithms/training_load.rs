...
impl TrainingLoadAlgorithm {
    ...
    ///
    /// ```rust,no_run
    /// use dravr_cageux::algorithms::training_load::{
    ///     TrainingLoadAlgorithm, TssDataPoint,
    /// };
    /// use chrono::{Duration, Utc};
    /// # fn example() -> Result<(), dravr_cageux::IntelligenceError> {
    ///
    ...
    /// # }
    /// ```
    pub fn calculate_ctl(&self, tss_data: &[TssDataPoint]) -> IntelligenceResult<f64> {
    ...
    /// - Dates are not properly ordered
    /// - Window sizes are invalid
    pub fn calculate_atl(&self, tss_data: &[TssDataPoint]) -> IntelligenceResult<f64> {
        if tss_data.is_empty() {
            return Ok(0.0);
        }
    ...
    ///
    /// Formula: `α = 2/(N+1)`, `EMA_t = α x TSS_t + (1-α) x EMA_{t-1}`
    fn calculate_ema(tss_data: &[TssDataPoint], window_days: i64) -> IntelligenceResult<f64> {
        if tss_data.is_empty() {
            return Ok(0.0);
        }
    ...
    ///
    /// Formula: `SMA = Σ(TSS_i) / N` for i in [t-N+1, t]
    fn calculate_sma(tss_data: &[TssDataPoint], window_days: i64) -> IntelligenceResult<f64> {
        if tss_data.is_empty() {
            return Ok(0.0);
        }
    ...
    fn calculate_wma(tss_data: &[TssDataPoint], window_days: i64) -> IntelligenceResult<f64> {
        if tss_data.is_empty() {
            return Ok(0.0);
        }
