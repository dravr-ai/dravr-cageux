...
impl FtpAlgorithm {
    ...
    /// # }
    /// ```
    pub fn estimate_ftp(&self) -> IntelligenceResult<f64> {
        match self {
    ...
    fn validate_power(power: f64, name: &str) -> IntelligenceResult<()> {
        if power <= 0.0 {
            return Err(IntelligenceError::invalid_input(format!(
    ...
    /// # Errors
    ///
    /// Returns `IntelligenceError::InvalidInput` if power or duration are non-positive
