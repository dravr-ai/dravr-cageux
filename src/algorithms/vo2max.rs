...
/// # Capture path
///
/// Every variant is driven by a measured field test — a 12-minute track run, a
/// timed mile walk with the finishing heart rate, steady-state ergometer watts
/// — and no provider feed supplies those inputs. They arrive in conversation:
/// the platform's `estimate_vo2max` tool takes the test the athlete describes,
/// builds the matching variant, calls [`Self::estimate_vo2max`], and the
/// athlete's `set_physiology` stores the number they confirm. Production
/// `VO2max` remains the profile value; these estimators feed it rather than
/// compete with it.
...
impl Vo2maxAlgorithm {
    ...
    /// # }
    /// ```
    pub fn estimate_vo2max(&self) -> IntelligenceResult<f64> {
        match self {
            Self::FromVdot { vdot } => {
    ...
    /// Validate VDOT value
    fn validate_vdot(vdot: f64) -> IntelligenceResult<()> {
    ...
    fn validate_gender(gender: u8) -> IntelligenceResult<()> {
        if gender > 1 {
            return Err(IntelligenceError::invalid_input(format!(
    ...
    fn calculate_cooper(distance_meters: f64) -> IntelligenceResult<f64> {
    ...
    fn calculate_rockport(data: RockportTestData) -> IntelligenceResult<f64> {
    ...
    fn calculate_from_pace(max_speed_ms: f64, recovery_speed_ms: f64) -> IntelligenceResult<f64> {
...
impl FromStr for Vo2maxAlgorithm {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            ...
            "astrand" | "astrand_ryhming" => Err(IntelligenceError::invalid_input(
            ...
            "from_pace" | "pace" => Err(IntelligenceError::invalid_input(
