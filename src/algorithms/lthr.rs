...
impl LthrAlgorithm {
    ...
    /// # }
    /// ```
    pub fn estimate_lthr(&self) -> IntelligenceResult<f64> {
        match self {
    ...
    fn validate_hr(hr: f64, name: &str) -> IntelligenceResult<()> {
    ...
    fn validate_percentage(percentage: f64) -> IntelligenceResult<()> {
    ...
    fn calculate_ramp_test_lthr(hr_samples: &[f64]) -> IntelligenceResult<f64> {
    ...
    /// # Errors
    ///
    /// Returns `IntelligenceError::InvalidInput` if HR values are outside physiological range
