...
impl TssAlgorithm {
    ...
    ///
    /// Returns `IntelligenceError::MissingData` if:
    /// - `NormalizedPower` algorithm selected but no power stream available
    ///
    /// # Example
    ///
    /// ```rust,no_run
    /// use dravr_cageux::algorithms::tss::TssAlgorithm;
    /// use dravr_cageux::models::Activity;
    /// use dravr_cageux::error::IntelligenceResult;
    ///
    /// # fn example(activity: &Activity) -> IntelligenceResult<()> {
    ...
    fn calculate_avg_power_tss(
        ...
        let avg_power = f64::from(
            activity
                .average_power()
                .ok_or_else(|| IntelligenceError::internal("average power data".to_owned()))?,
        );
    ...
    /// # Errors
    ///
    /// Returns `IntelligenceError::NotFound` if power stream is unavailable or too short
    fn calculate_normalized_power(
        ...
        let power_data = activity
            ...
            .ok_or_else(|| {
                IntelligenceError::internal(format!(
        ...
        if power_data.len() < window_size {
            return Err(IntelligenceError::internal(format!(
