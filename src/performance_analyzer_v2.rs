...
use crate::analysis_config::{AnalysisConfig, ConfidenceLevel};
use crate::config::intelligence::IntelligenceStrategy;
use crate::error::{IntelligenceError, IntelligenceResult};
use crate::metrics_extractor::{MetricType, SafeMetricExtractor};
use crate::models::Activity;
use crate::statistical_analysis::{RegressionResult, StatisticalAnalyzer};
use crate::types::{
...
impl PerformanceAnalyzerV2 {
    ...
    pub fn new(strategy: Box<dyn IntelligenceStrategy>) -> IntelligenceResult<Self> {
        let config = AnalysisConfig::from_environment().map_err(|e| {
            IntelligenceError::internal(format!("Configuration loading failed: {e}"))
        })?;
    ...
    pub fn analyze_trends(
        ...
        let filtered_activities = Self::filter_activities_by_timeframe(activities, &timeframe)
            .map_err(|e| {
                IntelligenceError::invalid_input(format!("Activity filtering failed: {e}"))
            })?;

        // 2. Extract metric values with type safety
        let metric_values =
            SafeMetricExtractor::extract_metric_values(&filtered_activities, metric_type).map_err(
                |e| IntelligenceError::internal(format!("Metric extraction failed: {e}")),
            )?;
        ...
        let regression = StatisticalAnalyzer::linear_regression(&data_points).map_err(|e| {
            IntelligenceError::internal(format!("Statistical analysis failed: {e}"))
        })?;
    ...
    pub fn calculate_fitness_score(
        &self,
        activities: &[Activity],
    ) -> IntelligenceResult<FitnessScore> {
    ...
    pub fn predict_performance(
        ...
        activities: &[Activity],
        target: &ActivityGoal,
    ) -> IntelligenceResult<PerformancePrediction> {
        ...
        let metric_type = Self::target_metric_to_metric_type(&target.metric)
            .map_err(|e| IntelligenceError::invalid_input(format!("Invalid metric type: {e}")))?;
        let metric_values =
            SafeMetricExtractor::extract_metric_values(&similar_activities, metric_type).map_err(
                |e| IntelligenceError::internal(format!("Metric extraction failed: {e}")),
            )?;
        ...
        .map_err(|e| {
            IntelligenceError::internal(format!("Confidence interval calculation failed: {e}"))
        })?;
    ...
    pub fn analyze_training_load(
        &self,
        activities: &[Activity],
    ) -> IntelligenceResult<TrainingLoadAnalysis> {
        ...
        let analysis = self.analyze_load_pattern(&weekly_loads).map_err(|e| {
            IntelligenceError::internal(format!("Load pattern analysis failed: {e}"))
        })?;
    ...
    fn filter_activities_by_timeframe(
        activities: &[Activity],
        timeframe: &TimeFrame,
    ) -> IntelligenceResult<Vec<Activity>> {
    ...
    fn target_metric_to_metric_type(metric: &str) -> IntelligenceResult<MetricType> {
        match metric.to_lowercase().as_str() {
            ...
            _ => Err(IntelligenceError::invalid_input(format!(
    ...
    fn analyze_load_pattern(
        &self,
        weekly_loads: &[WeeklyLoad],
    ) -> IntelligenceResult<LoadPatternAnalysis> {
        if weekly_loads.is_empty() {
            return Err(IntelligenceError::invalid_input(
                "No weekly load data to analyze",
            ));
        }
