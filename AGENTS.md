...
| Crate | Description |
|-------|-------------|
| `dravr-cageux` | Core library with traits, models, algorithms, and intelligence engine |
| `dravr-cageux-mcp` | MCP server exposing intelligence via Model Context Protocol |
| `dravr-cageux-server` | Unified REST API + MCP server + CLI binary |
...
cargo clippy -p dravr-cageux              # core library
cargo clippy -p dravr-cageux-mcp          # MCP server
cargo clippy -p dravr-cageux-server       # REST+MCP server
...
All errors MUST use `IntelligenceError` with appropriate variants:
```rust
// GOOD
return Err(IntelligenceError::InvalidInput { field: "age".into(), reason: "must be positive".into() });
return Err(IntelligenceError::InsufficientData { required: 7, actual: 3 });
